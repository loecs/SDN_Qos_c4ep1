...
class NetworkDelayDetector(app_manager.RyuApp):
    ...
    def get_delay(self, src, dst):
        try:
            t1 = self.awareness.graph[src][dst]['linkdelay']
            t2 = self.awareness.graph[dst][src]['linkdelay']
    ...
    def _save_link_delay(self, src=0, dst=0, linkdelay=0):
        try:
            self.awareness.graph[src][dst]['linkdelay'] = linkdelay
        except:
        ...
        try:
            ...
            for port in self.sw_module.ports.keys():
                if src_dpid == port.dpid and src_port_no == port.port_no:
                    ...
                    self._save_link_delay(src=src_dpid, dst=dpid,
                                          linkdelay=delay)
