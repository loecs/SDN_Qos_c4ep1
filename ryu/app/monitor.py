...
class Monitor(app_manager.RyuApp):
    ...
    def get_throughput(self, now, pre, period):
        if period:
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in sorted([flow for flow in body if flow.priority == 1],
            ...
            # Get flow's throughput.
            pre = 0
            period = setting.MONITOR_PERIOD
            tmp = self.flow_stats[dpid][key]
            if len(tmp) > 1:
                pre = tmp[-2][1]
                period = self.get_period(tmp[-1][2], tmp[-1][3],
                                          tmp[-2][2], tmp[-2][3])

            throughput = self.get_throughput(self.flow_stats[dpid][key][-1][1],
                                    pre, period)

            self._save_stats(self.flow_speed[dpid], key, throughput, 5)
    ...
    def show_stat(self):
        ...
        print('datapath             port   ''rx-pkts  rx-bytes rx-error '
              'tx-pkts  tx-bytes tx-error  port-throughput(b/s)  '
              'port-stat   link-stat')
        print('----------------   -------- ''-------- -------- -------- '
              '-------- -------- -------- '
              '----------------  ----------------   '
              '   -----------    -----------')
        format = '%016x %8x %8d %8d %8d %8d %8d %8d %8.1f %16s %16s'
            ...
            for stat in sorted(bodys[dpid], key=attrgetter('port_no')):
                if stat.port_no != ofproto_v1_3.OFPP_LOCAL:
                    print(format % (
                        ...
                        abs(self.port_throughput[(dpid, stat.port_no)][-1]),
